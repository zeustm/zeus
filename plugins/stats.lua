...
local function run(msg, matches)
  if matches[1]:lower() == 'zeus' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /zeus ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "zeus" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^[!/]([Ss]tatslist)$",
    "^[!/]([Ss]tats) (group) (%d+)",
    "^[!/]([Ss]tats) (zeus)",-- Put everything you like :)
		"^[!/]([Zz]eus)"-- Put everything you like :)
    }, 
  run = run
}
